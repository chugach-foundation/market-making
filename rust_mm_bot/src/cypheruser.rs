#![allow(dead_code)]
use crate::{fasttxnbuilder::FastTxnBuilder};
use anchor_lang::{ToAccountMetas, Owner, ZeroCopy};
use arrayref::array_ref;
use bytemuck::checked::from_bytes;
use crate::cyphergroup::CypherGroupInfo;
use cypher_tester::{WalletCookie, get_request_builder, derive_open_orders_address};
use cypher::{constants::B_CYPHER_USER, states::{CypherUser, CypherMarket, CypherToken}};
...
use solana_sdk::{
    ...
    rent::Rent,
    system_program,
    sysvar::SysvarId, signature::{Keypair}, signer::Signer, account::Account,
};
...
impl CypherMarketUser {
    pub async fn init(
        ...
        let group_cookie = CypherGroupInfo{ address : group_address, account_state: group };
    ...
    pub async fn get_deposit_collateral_ix(
        &self,
        amount: u64,
    ) -> Vec<Instruction> {
        let ixs = get_request_builder()
    ...
    pub async fn mint_and_sell(
        ...
        let ixs = get_request_builder()
            .accounts(
                self.get_new_order_v3_accounts()
            )
    ...
    pub async fn get_buy_and_burn_ixs(
        &self,
    ...
    pub async fn new_order_v3(
        &self,
        ix_data: NewOrderInstructionV3,
    ) -> Instruction {
        let accounts = self
            .get_new_order_v3_accounts();
    ...
    fn get_cancel_orders_accounts(
        ...
        vec![
            ...
            AccountMeta::new(identity(self.market_info.state.coin_vault).to_pubkey(), false),
    ...
    pub async fn cancel_order_v2(
        ...
        let accounts = self
            .get_cancel_orders_accounts(group,  true);
        let ix = Instruction {
            program_id: cypher::ID,
            accounts,
    ...
    pub async fn get_orders(
        &self,
        dex_oo_acc : Account
    ) -> Vec<CancelOrderInstructionV2> {
        let dex_open_orders: DexOpenOrders = {
            parse_dex_account(dex_oo_acc.data)
        };
        struct Iter {
            bits: u128,
        }
