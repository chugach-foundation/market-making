...
impl Slab {
  ...
  pub fn get_depth(
    ...
    let mut res: Vec<OrderBookLine> = Vec::with_capacity(depth_to_get);
    let leafs = self.get_leaf_depth(depth_to_get, is_asks).unwrap();

    for leaf in leafs {
        let token_price = u128::from(u64::from(leaf.price())) * u128::from(pc_lot_size) * u128::from(coin_token_decimal_factor) / u128::from(coin_lot_size);
        let token_quantity = leaf.quantity() * coin_lot_size;
        // Add total quantity
        let mut quantity_sum = if res.len() == 0 {
            0
        } else {
            res[res.len() - 1].quantity_sum
        };
        let mut price_quantity_sum = if res.len() == 0 {
            0
        } else {
            res[res.len() - 1].price_quantity_sum
        };
        quantity_sum += token_quantity;
        price_quantity_sum += u128::from(token_price) * u128::from(token_quantity);
        // Compute the prefix sum (sum of price * quantity)
        let line = OrderBookLine {
  ...
  #[inline]
  pub fn assert_minimum_capacity(&self, capacity: u32) {
      if self.nodes().len() <= (capacity as usize) * 2 {
          panic!("Slab too small!");
      }
  }
...
impl Slab {
  ...
  pub fn log_depth(&self, depth: usize, asc: bool) {
      let leafs = self.get_leaf_depth(depth, asc).unwrap();
    //   let res: Vec<OrderBookLine> = Vec::with_capacity(depth);
      for leaf in leafs {
        println!("LEAF NODE with price {}", leaf.price());
      }
  }
