...
use serum_dex::state::MarketStateV2 as DexMarketStateV2;
...
impl CypherGroupInfo {
    pub async fn load_dex_market(&self, market_idx: usize, rpc: &RpcClient) -> DexMarketStateV2 {
        let cypher_market = self.account_state.get_cypher_market(market_idx);
        let dex_market_acc = rpc.get_account(&cypher_market.dex_market).await.unwrap();
